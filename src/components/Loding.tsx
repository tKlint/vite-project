...
const Loading: React.FC<{
    ...
    const { style = {}, width = 0, height = 0 } = props;
    ...
    const renderCanvas = () => {
        ...
        for (let index = 0; index < 40; index += 1) {
            const [x, y] = createRandomPath(
                [0, window.innerWidth],
                [0, window.innerHeight]
            );
            const color = createRandomColor();
            let moveX = parseInt((Math.random() * 10).toString(), 10) - 5;
            let moveY = parseInt((Math.random() * 10).toString(), 10) - 5;
            moveX = moveX === 0 ? 1 : moveX;
            moveY = moveY === 0 ? 1 : moveY;
            const ball = new Ball({ x, y, r: 8, color, moveX, moveY, ctx });
